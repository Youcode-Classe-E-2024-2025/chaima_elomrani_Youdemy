...
<body>
    <div
        ...
        <div class="mt-3 text-center">
            ...
            <form id="updateCourseForm" class="mt-2 text-left"
                action="http://<?= $_SERVER['HTTP_HOST'] ?>/index.php?action=updateCourse" method="POST">
                <?php
                foreach ($courses as $course):
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="courseDescription" name="description" rows="6"
                            class="markdown-editor mt-1 block w-full rounded-md border-black shadow-sm focus:border-clair-500 focus:ring focus:ring-clair-500 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white"><?= htmlspecialchars($course['description']) ?></textarea>
                    </div>
                    ...
                    <input type="text" placeholder="Enter Price" value="<?= htmlspecialchars($course['price']) ?>"
                    ...
                    <div id="imageUrlInput" class="mb-4">
                        <label for="courseImage"
                            class="block text-sm py-4 font-medium text-gray-700 dark:text-gray-300">Course Image URL</label>
                        <input type="url" id="courseImage" name="image"
                            value="<?= htmlspecialchars($course['image_path']) ?>"
                    ...
                    <div id="videoUrlInput" class="mb-4">
                        <label for="courseVideo" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Course
                            Video URL</label>
                        <input type="url" id="courseVideo" name="video"
                            value="<?= htmlspecialchars($course['video_path']) ?>"
                            class="mt-1 py-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-clair-500 focus:ring focus:ring-clair-500 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    </div>
                    <?php endforeach; ?>
